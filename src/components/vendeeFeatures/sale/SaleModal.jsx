...
import createVendeeSaleAction, {
  updateVendeeSaleAction,
} from "@/actions/vendeeSaleAction";
...
export function SaleModal({
  ...
  async function submiteForm(newData) {
    ...
    startTransition(async () => {
      if (type === "create") {
        const result = await createVendeeSaleAction(myNewData);
      ...
      if (type === "update") {
        ...
        const result = await updateVendeeSaleAction(currentData, myNewData);
        if (result.result?.message)
          return toast.warning(result.result?.message);
        if (!result.err) {
