...
import {
  createVendeeReceiveAction,
  updateVendeeReceiveAction,
} from "@/actions/vendeeReceiveAction";

const schema = z.object({
  date: z
    .date({ required_error: "تاریخ الزامی میباشد" })
    .default(() => new Date()),
  buyer: z.string({ required_error: "خریدار الزامی میباشد" }),
  amount: z
...
export function ReceiveModal({
  ...
  async function submiteForm(newData) {
    ...
    startTransition(async () => {
      if (type === "create") {
        const result = await createVendeeReceiveAction(myNewData);
      ...
      if (type === "update") {
        const currentData = { ...data, buyer: data.buyer._id };
        const result = await updateVendeeReceiveAction(currentData, myNewData);
