import { deleteApplecationAction } from "@/actions/applecationAction";
...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteApplecationAction(data); // or whatever your delete logic is

      if (result.result?.message) return toast.warning(result.result?.message);
      if (!result.err) {
        toast.success("مصرف شما با موفقیت حذف شد");
      } else {
        toast.error(
          "در حذف مصرف شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
        );
      }
      ...
      <DialogContent className=" flex flex-col text-right items-start">
        <DialogHeader className=" flex flex-col t items-start">
          ...
          <DialogDescription className={"text-right"}>
            آیا مطمئن هستید اطلاعات مصرف {data.costTitle} را حذف کنید؟.
          </DialogDescription>
        </DialogHeader>
