...
export const columns = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="lowercase">{row.getValue("type")?.name}</div>
    ),
  },
  {
    accessorKey: "income",
    ...
    cell: ({ row }) => {
      const name = row.getValue("income")?.name;
      return (
        <div className="lowercase">{name === "company" ? "شرکت" : name}</div>
      );
    },
  },
  {
...
export function DataTableReceive({ data, count }) {
        ...
        <div className="flex gap-4">
          ...
          <ReceiveModal open={open} onOpen={setOpen}>
            <DialogTrigger asChild>
              <Button>
                افزودن دریافتی <ShoppingBasket size={32} strokeWidth={1.75} />
              </Button>
            </DialogTrigger>
          </ReceiveModal>
        </div>
