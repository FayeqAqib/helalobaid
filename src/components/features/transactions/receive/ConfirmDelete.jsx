import { deleteReceiveAction } from "@/actions/receiveAction";
...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteReceiveAction(data); // or whatever your delete logic is
      ...
      <DialogContent className=" flex flex-col text-right items-start">
        <DialogHeader className=" flex flex-col t items-start">
          ...
          <DialogDescription className={"text-right"}>
            آیا مطمئن هستید اطلاعات معامله با {data.type.name} را حذف کنید؟.
          </DialogDescription>
        </DialogHeader>
