import { deleteTransferAction } from "@/actions/transferAction";
...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteTransferAction(data); // or whatever your delete logic is
      ...
      <DialogContent className=" flex flex-col text-right items-start">
        <DialogHeader className=" flex flex-col t items-start">
          ...
          <DialogDescription className={"text-right"}>
            آیا مطمئن هستید اطلاعات انتقال پول از {data.from.name} به
            {data.to.name} را حذف کنید؟.
          </DialogDescription>
        </DialogHeader>
