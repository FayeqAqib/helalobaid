...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteUserAction(data._id, data.owner._id); // or whatever your delete logic is

      if (result.result?.message) return toast.warning(result.result?.message);
      if (!result.err) {
        toast.success(" شما با موفقیت حذف شد");
      } else {
        toast.error(
      ...
      <DialogContent className=" flex flex-col text-right items-start">
        <DialogHeader className=" flex flex-col t items-start">
          ...
          <DialogDescription className={"text-right"}>
            آیا مطمئن هستید اطلاعات معامله با {data?.owner?.name} را حذف کنید؟.
          </DialogDescription>
        </DialogHeader>
