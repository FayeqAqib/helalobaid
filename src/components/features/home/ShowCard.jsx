...
export const ShowCard = ({ tital, count, amount, cent = 1 }) => {
  return (
    <Card className={"p-3 shadow-lg flex  w-full max-w-lg "}>
      <CardHeader className={"text-right flex flex-row justify-between p-0"}>
        <div className="flex flex-row items-center justify-center gap-2">
          <img src="../total-earn.svg" className="w-10 h-10" />
          <CardTitle className={"text-xl font-extrabold"}>{tital}</CardTitle>
        </div>
        <CardTitle className={"text-xl m-1 font-extrabold"}>{count}</CardTitle>
      </CardHeader>
      <CardContent className={"text-right flex flex-row justify-between p-0"}>
        <div className="flex flex-col gap-2 m-x-1 mt-3">
          ...
          {cent != 1 && (
            <CardDescription className={"flex flex-row text-xs gap-2"}>
              ...
              <span className={cent > 0 ? "text-green-500" : "text-red-500"}>
        ...
        <img src="../download.png" className="relative rotate-y-[180deg]  " />
      </CardContent>
    </Card>
  );
};
