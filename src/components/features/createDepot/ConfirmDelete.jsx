...
import { deleteDepotAction } from "@/actions/depotAction";
...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteDepotAction(data._id); // or whatever your delete logic is
      ...
      <DialogContent className=" flex flex-col text-right items-start">
        <DialogHeader className=" flex flex-col t items-start">
          ...
          <DialogDescription className={"text-right"}>
            آیا مطمئن هستید گدام {data.name} را حذف کنید؟.
          </DialogDescription>
        </DialogHeader>
