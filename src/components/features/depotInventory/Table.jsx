...
export const columns = [
...

  {
    ...
    cell: ({ row }) => (
      <div className="lowercase">{row.getValue("depot").name}</div>
    ),
  },
  {
    accessorKey: "details",
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const [openDepotInventory, setOpenDepotInventory] = useState(false);
      const [openDetails, setOpenDetails] = useState(false);
      return (
        <DepotInventoryModal
          key={openDepotInventory}
          data={payment}
          type="update"
          open={openDepotInventory}
          onOpen={setOpenDepotInventory}
        >
          <ConfirmDelete
            data={payment}
            open={openDelete}
                  ...
                  {payment.createBy !== "buy" && (
                    <>
                      ...
                      >
                        <Button
                          onClick={() =>
                            setOpenDepotInventory(
                              (openDepotInventory) => !openDepotInventory
                            )
                          }
                          variant={"ghost"}
                          className={"w-full justify-end"}
...
export function DataTableDepotInventory({ data, count }) {
        ...
        <div className="flex gap-4">
          ...
          <DepotInventoryModal open={open} onOpen={setOpen}>
            <DialogTrigger asChild>
              <Button>
                افزودن موجودی <HiCurrencyRupee className="size-6" />
              </Button>
            </DialogTrigger>
          </DepotInventoryModal>
        </div>
          ...
          <TableFooter>
            ...
            >
              {table.getFooterGroups()[0].headers.map((footer, idx) => (
                ...
                >
                  {["transportCost", "unitAmount"].includes(footer.id)
                    ? formatCurrency(
                        table
                          .getRowModel()
                          .rows.reduce(
