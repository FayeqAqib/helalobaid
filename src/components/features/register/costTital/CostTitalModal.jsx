...
const schema = z.object({
  ...
  name: z.string({ required_error: '" ذکر نام کتگوری مصارف الزامی است "' }),
  details: z.string().optional(),
});

export function CostTitalModal({
  children,
  data = {},
  type = "create",
  open,
  ...
  async function submiteForm(formData) {
    ...
    startTransition(async () => {
      if (type === "create") {
        const result = await createCostTitalAction(newFormData);
        if (!result.err) {
          toast.success("کتگوری  شما با موفقیت ایجاد شد");
          form.reset();
          onOpen(false);
        } else {
          toast.error(
            "در ایجاد کتگوری  شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
          );
        }
      }
      if (type === "update") {
        const result = await updateCostTitalAction({
          ...newFormData,
          _id: data._id,
        });

        if (!result.err) {
          toast.success("کتگوری  شما با موفقیت آپدیت شد");
          form.reset();
          onOpen(false);
        } else {
          toast.error(
            "در آپدیت کتگوری  شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
          );
        }
      }
    });
      ...
      <DialogContent className=" lg:w-[600px]">
        <DialogHeader>
          <DialogTitle className={"text-right"}>
            {type == "update" ? "تصحیح" : " کتگوری مصارف جدید "}
          </DialogTitle>
