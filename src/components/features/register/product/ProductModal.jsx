...
export function ProductModal({
  ...
  async function submiteForm(formData) {
    ...
    startTransition(async () => {
      if (type === "create") {
        const result = await createProductAction(newFormData);
        if (!result.err) {
          ...
          toast.error(
            "در ایجاد محصول شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
          );
        }
      }
      if (type === "update") {
        ...
        if (!result.err) {
          toast.success("محصول شما با موفقیت آپدیت شد");
          form.reset();
          onOpen(false);
        } else {
          toast.error(
            "در آپدیت محصول شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
          );
        }
      }
    });
