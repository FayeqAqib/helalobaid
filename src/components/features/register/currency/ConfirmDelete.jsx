...
import { deleteCurrencyAction } from "@/actions/currencyAction";
...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteCurrencyAction(data._id); // or whatever your delete logic is

      if (result.result?.message) return toast.warning(result.result?.message);
      if (!result.err) {
        toast.success("واحد پولی شما با موفقیت حذف شد");
      } else {
        toast.error(
          "در حذف واحد پولی شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
        );
      }
      onOpen(false);
    });
