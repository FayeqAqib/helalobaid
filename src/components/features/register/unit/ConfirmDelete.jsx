...
import { deleteUnitAction } from "@/actions/unitAction";
...
export default function ConfirmDelete({ children, data, open, onOpen }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteUnitAction(data._id); // or whatever your delete logic is

      if (result.result?.message) return toast.warning(result.result?.message);
      if (!result.err) {
        toast.success("واحد شما با موفقیت حذف شد");
      } else {
        toast.error(
          "در حذف واحد شما مشکلی به وجود آمده لطفا بعدا دوباره تلاش کنید"
        );
      }
      onOpen(false);
    });
    ...
    <Dialog open={open} onOpenChange={onOpen}>
      ...
      <DialogContent className=" flex flex-col text-right items-start">
        <DialogHeader className=" flex flex-col t items-start">
          <DialogTitle>حذف واحد</DialogTitle>
          <DialogDescription className={"text-right"}>
            آیا مطمئن هستید اطلاعات واحد با نام {data.name} را حذف کنید؟.
          </DialogDescription>
        </DialogHeader>
